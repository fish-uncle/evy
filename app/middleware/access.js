...
module.exports = _ => {
  return async function (ctx, next) {
    const {access = {ignore: []}} = ctx.app.config;
    const skipExt = access.ignore;
    ...
    if (!isSkip) {
      ...
      const message = util.format('%s:%s - %s %s %s/%s %s %s %s %s %s', ip, port, method, url, protocol, status, length, referrer, rs, serverTime, ua);
      accessLogger.info(message);
      const query = JSON.stringify(ctx.query), body = JSON.stringify(ctx.request.body);
      if (query !== '{}' || body !== '{}') {
